    ...
    return (
        <div className="min-h-screen bg-[var(--background)] flex flex-col md:flex-row h-screen overflow-hidden">
            ...
            <div className="w-full md:w-[50%] lg:w-[55%] h-[60vh] md:h-full relative flex items-center justify-center overflow-hidden bg-white dark:bg-[#111] md:rounded-r-3xl md:shadow-2xl z-10">
                ...
                <div className="relative w-full h-full bg-gray-100 dark:bg-[#111] flex flex-col items-center justify-center p-4">
                    {activeSlide.type === 'photo' ? (
                        ...
                        <div className="w-full h-full relative p-0 flex items-center justify-center bg-gray-50 dark:bg-[#111]">
                            {/* @ts-ignore */}
                            {(activeSlide.outfit.imageUrl || activeSlide.outfit.image_url) ? (
                                <Image
                                    // @ts-ignore
                                    src={activeSlide.outfit.imageUrl || activeSlide.outfit.image_url}
                                ...
                                <div className="w-full aspect-square max-w-sm grid grid-cols-2 gap-[2px] bg-gray-200 dark:bg-gray-700">
                                    {/* @ts-ignore */}
                                    {(activeSlide.outfit.outfit_items || []).slice(0, 4).map((item: any, i: number) => {
                                    ...
                                    {/* @ts-ignore */}
                                    {Array.from({ length: Math.max(0, 4 - (activeSlide.outfit.outfit_items?.length || 0)) }).map((_, i) => (
                                        <div key={`empty-${i}`} className="bg-gray-50 dark:bg-gray-800 rounded-2xl aspect-square border border-[var(--border-color)] opacity-50" />
                                    ))}
                                </div>
                            )}
                        </div>
                        ...
                        <div className="absolute bottom-6 left-1/2 -translate-x-1/2 flex gap-2 z-10 bg-black/20 backdrop-blur-md px-3 py-1.5 rounded-full">
                            {slides.map((_, idx) => (
                                <button
                                    ...
                                    className={`h-2 rounded-full transition-all duration-300 ${activeImage === idx ? 'bg-white w-6' : 'bg-white/50 w-2 hover:bg-white/80'}`}
            ...
            <div className={`md:w-[50%] lg:w-[45%] h-[40vh] md:h-full bg-[var(--background)] flex flex-col overflow-hidden`}>

                {/* STICKY ACTION BAR */}
                <div className="sticky top-0 z-20 bg-[var(--background)]/90 backdrop-blur-md px-6 md:px-12 py-4 border-b border-[var(--border-color)] flex items-center justify-between">
                    <div className="flex items-center gap-6 text-[var(--foreground)]">
                        <button onClick={toggleLike} className="flex items-center gap-2 font-bold hover:opacity-80 transition-opacity">
                            <Heart className={`w-7 h-7 transition-colors ${isLiked ? 'fill-[var(--brand-pink)] text-[var(--brand-pink)]' : ''}`} strokeWidth={2.5} />
                    ...
                    <button onClick={toggleSave} className={`px-6 py-2.5 rounded-full font-bold text-sm transition-colors ${isSaved ? 'bg-[var(--background-secondary)] text-[var(--foreground)]' : 'bg-[var(--foreground)] text-[var(--background)] hover:opacity-80'}`}>
                        {isSaved ? 'Guardado' : 'Guardar'}
                    </button>
                </div>

                {/* SCROLLABLE CONTENT (Dynamic based on Slide) */}
                <div className="flex-1 overflow-y-auto px-6 md:px-12 pb-24 custom-scrollbar">
                    {activeSlide.type === 'photo' ? (
                        <>
                            {/* Photo Stats / User Header */}
                            <div className="flex items-center justify-between mt-6">
                                <Link href={`/profile/${author.id}`} className="flex items-center gap-3">
                                    <div className="relative w-10 h-10 rounded-full overflow-hidden bg-[var(--background-secondary)] border border-[var(--border-color)]">
                                        <Image src={author.avatar_url || 'https://i.pravatar.cc/150'} alt={author.username} fill className="object-cover" />
                                    </div>
                                    <div>
                                        <span className="font-bold text-[var(--foreground)] text-base hover:underline">{author.username}</span>
                                        {/* @ts-ignore */}
                                ...
                                {user?.id !== post.user_id && (
                                    <button onClick={toggleFollow} className={`px-4 py-1.5 rounded-full font-bold text-xs transition-all ${isFollowing ? 'bg-[var(--background-secondary)] text-[var(--foreground)]' : 'bg-[var(--brand-pink)] text-white hover:bg-[var(--brand-pink-dark)]'}`}>
                            ...
                            {post.caption && (
                                <div className="mt-4 mb-6">
                                    {/* @ts-ignore */}
                                    <p className="text-[15px] text-[var(--foreground)] whitespace-pre-wrap">{post.caption}</p>
                                </div>
                            )}

                            {/* Desk-only comment display or Mobile-when-opened */}
                            <div className={`mt-6 flex-col overflow-hidden ${showMobileComments ? 'flex' : 'hidden md:flex'}`}>
                                <h2 className="font-bold text-lg text-[var(--foreground)] mb-4">Comentarios</h2>
                                <div className="space-y-5">
                                    {comments.length === 0 ? (
                                        <div className="text-center py-6 text-[var(--foreground-tertiary)]">
                                            ...
                                            <div key={comment.id} className="flex gap-3">
                                                <Link href={`/profile/${comment.user.id}`} className="flex-shrink-0">
                                                    <div className="w-8 h-8 rounded-full bg-[var(--background-secondary)] overflow-hidden relative">
                                                        <Image src={comment.user.avatar_url || 'https://i.pravatar.cc/150'} alt={comment.user.username} fill className="object-cover" />
                                                    </div>
                                                </Link>
                                                <div className="flex-1">
                                                    <div className="flex gap-2 items-baseline">
                                                        <Link href={`/profile/${comment.user.id}`} className="font-bold text-[var(--foreground)] text-sm hover:underline">
                                                            {comment.user.username}
                                                        </Link>
                                                        <span className="text-[var(--foreground-secondary)] text-sm">{comment.content}</span>
                                                    </div>
                                                    <div className="flex gap-4 mt-1 text-xs font-bold text-[var(--foreground-tertiary)]">
                                                ...
                                                <button className="flex-shrink-0 self-start mt-1 cursor-pointer hover:opacity-70">
                                                    <Heart className="w-3.5 h-3.5 text-[var(--foreground-tertiary)]" />
                                                </button>
                                            </div>
                                        ))
                                    )}
                ...
                {/* Desktop Sticky Add Comment Input (Only in Photo Slide) */}
                {activeSlide.type === 'photo' && (
                    <form onSubmit={handleAddComment} className={`bg-[var(--background)] px-6 md:px-12 py-4 border-t border-[var(--border-color)] z-30 hidden md:flex gap-3 items-center sticky bottom-0`}>
                        <div className="w-9 h-9 rounded-full bg-gray-200 overflow-hidden relative flex-shrink-0 border border-[var(--border-color)]">
                        ...
                        <div className="relative flex-1">
                            <input
                                ...
                                className="w-full bg-[var(--background-secondary)] rounded-full px-5 py-2.5 text-sm font-medium outline-none border border-transparent focus:border-[var(--border-color)] transition-all placeholder:text-[var(--foreground-tertiary)]"
                                value={newComment}
                        ...
                        <button
                            ...
                            className="text-[var(--brand-pink)] font-bold text-sm hover:text-[var(--brand-pink-dark)] disabled:opacity-50 transition-colors px-2"
                        >
                            Publicar
                        </button>
                    </form>
            ...
            {selectedItem && (
                ...
                >
                    <div
                        className="bg-[var(--card-bg)] rounded-t-3xl md:rounded-3xl max-w-md w-full p-6 space-y-4 mb-16 md:mb-0 shadow-2xl relative animate-in slide-in-from-bottom-10 md:fade-in-10"
                        onClick={(e) => e.stopPropagation()}
                    >
                        <button
                            onClick={() => setSelectedItem(null)}
                            className="absolute top-4 right-4 p-2 bg-[var(--background-secondary)] hover:bg-[var(--border-color)] rounded-full transition-colors z-10"
                        >
                            <X className="w-5 h-5 text-[var(--foreground)]" />
                        </button>

                        <div className="relative w-full aspect-square rounded-2xl overflow-hidden bg-[var(--background-secondary)]">
                        ...
                        <div>
                            ...
                            <p className="text-[var(--foreground-secondary)] font-medium mt-1">{selectedItem.brand || 'Sin marca'}</p>
                            <p className="text-sm text-[var(--foreground-tertiary)] uppercase tracking-wider mt-2">{selectedItem.category}</p>
                            {selectedItem.color && (
                                <div className="flex items-center gap-2 mt-3 p-3 bg-[var(--background-secondary)] rounded-xl w-fit">
                                    <span className="text-xs font-semibold text-[var(--foreground-secondary)]">Color principal:</span>
                                    <div
                                        className="w-4 h-4 rounded-full border border-[var(--border-color)] shadow-sm"
                        ...
                        <button
                            onClick={() => window.location.href = `/closet?item=${selectedItem.id}`}
                            className="w-full py-3.5 mt-4 rounded-full bg-[var(--brand-pink)] text-white font-bold tracking-wide hover:bg-[var(--brand-pink-dark)] transition-colors"
                        >
                            Ver en mi armario
                        </button>
                    </div>
