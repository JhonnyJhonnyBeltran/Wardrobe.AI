...
export default function CreateOutfitPage() {
    ...
    const handleSaveOutfit = () => {
        console.log('Saving outfit...', { isPublic });
        // TODO: Aquí se guardará el outfit con el campo isPublic en la base de datos
    };
                            ...
                            <AnimatePresence mode="wait">
                                {generatedOutfit.length === 0 ? (
                                    ...
                                    >
                                        ...
                                        {/* Public Toggle */}
                                        <div className="flex items-center justify-between p-4 rounded-xl bg-[var(--background-secondary)] border border-[var(--border-color)]">
                                            <div>
                                                <p className="font-medium text-[var(--foreground)] text-sm">
                                                    Hacer público
                                                </p>
                                                <p className="text-xs text-[var(--foreground-tertiary)] mt-0.5">
                                                    Mostrar este outfit en tu perfil y en el feed
                                                </p>
                                            </div>
                                            <button
                                                onClick={() => setIsPublic(!isPublic)}
                                                className={`relative w-12 h-6 rounded-full transition-colors ${isPublic ? 'bg-[var(--brand-pink)]' : 'bg-[var(--background-tertiary)]'
                                                    }`}
                                            >
                                                <motion.div
